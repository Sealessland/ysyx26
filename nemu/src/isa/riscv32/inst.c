...
#define immB()                                                                 \
  do {                                                                         \
    *imm = ((SEXT(BITS(i, 31, 31), 1) << 11) | BITS(i, 7, 7) << 10 |           \
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */)                   \
  ...
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt, B,
          s->dnpc = ((int32_t)src1 < (int32_t)src2) ? s->pc + SEXT(imm, 12)
                                                    : s->dnpc);
  INSTPAT("??????? ????? ????? 110 ????? 11000 11", bltu, B,
          s->dnpc = (src1 < src2) ? s->pc + SEXT(imm, 12) : s->dnpc);
  INSTPAT("??????? ????? ????? 111 ????? 11000 11", bgeu, B,
          (src1 >= src2) ? (s->dnpc = s->pc + imm) : 0);
  INSTPAT("0000001 ????? ????? 000 ????? 01100 11", mul, R,
          R(rd) = src1 * src2);
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh, R,
          R(rd) = ((int64_t)SEXT(src1, 32) * (int64_t)SEXT(src2, 32)) >> 32);
  INSTPAT("0000001 ????? ????? 010 ????? 01100 11", mulhsu, R,
          R(rd) = ((int64_t)SEXT(src1, 32) * SEXT(src2, 32)) >> 32);
