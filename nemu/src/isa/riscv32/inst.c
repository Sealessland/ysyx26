 ...
 * PSL v2. You may obtain a copy of Mulan PSL v2 at: http://license.coscl.org.cn/MulanPSL2
 *******/
...
enum { TYPE_I, TYPE_U, TYPE_S, TYPE_R, TYPE_B, TYPE_N, TYPE_J };
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */) { int rd = 0; word_t src1 = 0, src2 = 0, imm = 0; decode_operand(s, &rd, &src1, &src2, &imm, concat(TYPE_, type)); __VA_ARGS__; }
  ...
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw  , I, word_t t = CSR(imm & 0xfff); CSR(imm & 0xfff) = src1; R(rd) = t);
  INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs  , I, word_t t = CSR(imm & 0xfff); CSR(imm & 0xfff) = t | src1; R(rd) = t);
  INSTPAT("??????? ????? ????? 011 ????? 11100 11", csrrc  , I, word_t t = CSR(imm & 0xfff); CSR(imm & 0xfff) = t & ~src1; R(rd) = t);
  INSTPAT("??????? ????? ????? 101 ????? 11100 11", csrrwi , I, word_t t = CSR(imm & 0xfff); CSR(imm & 0xfff) = BITS(s->isa.inst, 19, 15); R(rd) = t);
  INSTPAT("??????? ????? ????? 110 ????? 11100 11", csrrsi , I, word_t t = CSR(imm & 0xfff); CSR(imm & 0xfff) = t | BITS(s->isa.inst, 19, 15); R(rd) = t);
  INSTPAT("??????? ????? ????? 111 ????? 11100 11", csrrci , I, word_t t = CSR(imm & 0xfff); CSR(imm & 0xfff) = t & ~BITS(s->isa.inst, 19, 15); R(rd) = t);
